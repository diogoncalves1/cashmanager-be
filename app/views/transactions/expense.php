...
<main class="col-md-9 ms-sm-auto col-lg-10 px-md-4">
    <div
        ...
        <h1 class="h2"><?= $translate["add_expense"]; ?></h1>
                ...
                <form class="form-card" method="POST" enctype="multipart/form-data">
                    <div class="row justify-content-between text-left">
                        ...
                        <div class="form-group col-sm-6 flex-column d-flex">
                            ...
                            <input type="date" max="<?= $currentDate; ?>" class="input" name="date" required>
                        </div>
                    </div>
                    <div class="row justify-content-between text-left">
                        <div class="form-group col-sm-12 flex-column d-flex"> <label
                            ...
                            <select name="category" required>
                                <?php foreach ($categorys as $category) {
                                    if ($category['id'] != 33) {
                                        if ($category['sub-category'] == 1) { ?>
                                            </optgroup>
                                            <optgroup label="<?= $translate[$category["code"]] ?>">
                                            <?php } ?>
                    ...
                    <div class="row justify-content-between text-left">
                        <div class="form-group col-sm-6 flex-column d-flex"> <label
                                class="form-control-label px-3"><?= $translate["account"]; ?><span
                                    class="text-danger-emphasis"> *</span></label>
                        ...
                        <div class="form-group col-sm-6 flex-column d-flex pb-3">
                            <label class="form-control-label px-3"><?= $translate["to"]; ?>:
                                <span>(<?= $translate["optional"]; ?>)</span>
                            </label>
                            <input type="text" class="input" name="to" placeholder="<?= $translate["to"]; ?>...">
                        </div>
                    </div>
                    ...
                    <div class="row">
                        <div class="form-group col flex-column d-flex pb-3">
                            <label for=""><?= $translate["proof_of_payment"]; ?> <span>(<?= $translate["optional"] ?>)</span></label>
                    ...
                    <div class="row justify-content-end">
                        <div class="form-group col-sm-6">
                            <button type="submit"
                                class="button btn btn-sm btn-outline-danger"><?= $translate["add_expense"]; ?></button>
                        </div>
                    </div>
                </form>
            </div>
