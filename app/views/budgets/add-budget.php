                    ...
                    <div class="row justify-content-between text-left">
                        <div class="row" id="input-container">
                            <div class="form-group col-sm-6 flex-column d-flex"> <label
                                    class="form-control-label px-3  "> Max <span
                            ...
                            <div class="form-group col-sm-6 flex-column d-flex"> <label
                                ...
                                <select name="period[]" class="select-type" onchange="catTest()">
                                    <?php $result = get_time_period($conn);
                                    while ($row = $result->fetch_assoc()) { ?>
                                        <option value="<?= $row['id'] - 1; ?>">
...
<script>
    ...
    function addLine() {
        var text = '<div class="form-group col-sm-6 flex-column d-flex"> <label class="form-control-label px-3  "> Max <span class="text-danger-emphasis"> *</span></label><input type="number" class="input" min="0.01" step="0.01" id="input-value" name="value[]" placeholder="Max: 0.00' + '<?= $coin ?>' + '" required></div><div class="form-group col-sm-6 flex-column d-flex" > <label class="form-control-label px-3  "> <?= $period ?> <span class="text-danger-emphasis"> *</span></label><select name="period[]" class="select-type" onchange="catTest()">';
        <?php $result = get_time_period($conn);
        while ($row = $result->fetch_assoc()) { ?>
            text += '<option value="<?= $row['id'] - 1; ?>">';
            <?php if ($_COOKIE['lang'] == "EN") { ?>
                text += '<?= $row['name']; ?>';
            <?php } else { ?>
                text += '<?= $row['name_pt']; ?>';
            ...
            text += '</option >';
        <?php } ?>
        text += '</select></div > <div class="form-group col-12 flex-column d-flex" ><label class="form-control-label px-3"><?= $category; ?><span class="text-danger-emphasis"> *</span ></label ><select class="select" name="category[]" onchange="catTest()">';
        <?php $result_cats = get_all_category($conn);
        while ($row = mysqli_fetch_assoc($result_cats)) {
            if ($row['id'] != 33) {
                if ($row['sub-category'] == 1) { ?>
