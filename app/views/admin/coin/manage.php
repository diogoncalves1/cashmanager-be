...
?>
    <main class="col-md-9 ms-sm-auto col-lg-10 px-md-4">
        ...
        <div
            ...
            <h1 class="h2"><?php echo $words[$_COOKIE["lang"]]["coins"]; ?></h1>
        </div>

        <button class="btn btn-outline-primary mb-4" onclick="goToAdd()">
            <?= $words[$_COOKIE["lang"]]["add_coin"] ?>
        </button>
        <div class="table-responsive center">
            <table class="table table-sm table-striped align-middle text-center">
                <thead>
                    <tr>
                        <th scope="col" class="col-4"><?php echo $words[$lang]["name"]; ?></th>
                        <th scope="col" class="col-3"><?php echo $words[$lang]["symbol"]; ?></th>
                        <th scope="col" class="col-3"><?php echo $words[$lang]["code"]; ?></th>
                ...
                <tbody id="table">
                    ...
                    foreach ($coins as $coin) { ?>
                        <tr id="category-<?= $coin['id'] ?>"
                            class="<?php if ($_COOKIE['mode'] == "light") { ?>table-primary <?php } ?>primary">
                            <td><?= $words[$lang][$coin["name"]]; ?></td>
                            <td><?= $coin["symbol"] ?></td>
                            <td><?= $coin["code"] ?></td>
                            <td><button style="background: none; border: none;" onclick="goToEdit(<?php echo $coin['id']; ?>)">
                                    <svg class="bi">
                                        <use xlink:href="#edit" />
                                    </svg>
                                </button>
                            ...
                            <td class="center"><button data-bs-toggle="modal" data-bs-target="#delete-modal"
                                    style="background: none; border: none;" onclick="updateModal(<?php echo $coin['id']; ?>)">
                                    <svg class="bi">
    ...
    <script src="/CashManager/public/assets/admin/js/coins.js"></script>
<?php } ?>